...
describe LookerSDK::Client::Domains do
  ...
  describe ".domain", :vcr do
    it "retrives single domain" do
      ...
      fetched_domain = LookerSDK.domain(domain.id)
  ...
  describe ".create_domain", :vcr do
    it "creates domain with models list" do
      ...
      domain = LookerSDK.create_domain(:name => "test_domain", :models => models)
    ...
    it "rejects duplicate name" do
      ...
      assert_raises LookerSDK::UnprocessableEntity do
        LookerSDK.create_domain(:name => domain.name, :models => models)
      end
      # clean up domain
      LookerSDK.delete_domain(domain.id).must_equal true
    end
  ...
  describe ".update_domain", :vcr do
    it "updates name" do
      ...
      domain = LookerSDK.update_domain(domain.id, {:name => "new_test_domain"})
    ...
    it "updates domain from all to limited" do
      ...
      new_models = ["foo", "bar"]
      domain = LookerSDK.update_domain(domain.id, {:models => new_models})
      domain.all_access.must_equal false
  ...
  describe ".delete_domain", :vcr do
    ...
    it "will not delete (403) built in all domain" do
      ...
      assert_raises LookerSDK::Forbidden do
        LookerSDK.delete_domain(all_domain.id)
      end
    end
  end
